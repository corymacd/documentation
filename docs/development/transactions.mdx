...
- Exactly-once semantics helps developers avoid the anomalies of at-most-once processing (with potential duplicated events) and at-least-once processing (with potential lost events). Redpanda supports exactly-once semantics with [Idempotent Producers](/docs/development/idempotent-producers). 
...
```
   ...
   while (true) {
       ...
       var sender_branch = ...;
       var sender_account = ...;
       var recepient_branch = ...;
       var recepient_account = ...;
       var amount = 42;
       ...
       try {
           ...
           var f2 = producer.send(new ProducerRecord<String, String>("ledger", recepient_branch, recepient_account, "" + amount));
           f1.get();
           f2.get();
       } catch (Exception e1) {
           // TODO: log error for further analysis
...
The majority of the code is error handling. When a tx fails before the `commitTransaction` attempt, it's safe to assume that it wasn't executed. When it fails after the `commitTransaction` attempt, the true status is unknown. Redpanda only guarantees that there isn't a partial result: either the transaction is committed, or it is fully rolled back.

### Consume-transform-produce loop

Redpanda is commonly used as a pipe connecting different data storage. An app could use an OLTP database and then rely on change data capture to deliver the changes to an analytical warehouse. Transactions turn Redpanda into a smart pipe. Instead of just opaque data transfering, the system can do transformations on the data, like lookups or aggregations.
...
In the multi-partition example, the request was coming from outside the system, and it was the app's responsibility to discover the true status of a timed out transaction. 

The consume-transform-loop example, however, is a closed system. Upon re-initialization of the consumer and producer, the system automatically discovers the moment it was interrupted and continues from that place.

Additionally, the consume-transform-loop automatically scales by the number of partitions. Run another instance of the app, and it starts processing its share of partitions in the source topic.
