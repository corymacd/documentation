...
When a transaction fails before the `commitTransaction` attempt, it's safe to assume that it was not executed. When it fails after the `commitTransaction` attempt, the true status is unknown. Redpanda only guarantees that there isn't a partial result: either the transaction is committed, or it is fully rolled back.
