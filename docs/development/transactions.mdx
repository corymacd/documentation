...
Idempotence must be enabled to use transactions. By default, the following cluster properties are enabled: `enable_idempotence` and `enable_transactions`.
...
## Questions for Denis/Matt
- To enable transactions, does anything else need to be done to set up the producer/client side? e.g., do users need to do/know anything about transactional.id? 
- Is the default still 1 for `id_allocator_replication` and `transaction_coordinator_replication`?
- Can you provide an example for Using transactions? 
- Is there a use case for wanting to disable transactions? 