...
| :-------- | :----------- |
| node_id | A number that identifies the node within the cluster. This value can be assigned automatically, or it can be assigned by the user. Each node must have a unique node_id, and nodes should be numbered in sequence. This property must not be changed once a node joins a cluster. <br/><br/>For more information, see [Instantiate a cluster](../../deployment/production-deployment/#instantiate-a-cluster). <br/><br/>Default: Null |
...
| seed_servers | List of the seed servers used to join the current cluster. If the seed_servers list is empty, the node will be a cluster root and it will form a new cluster. <br/><br/>For more information, see [Instantiate a cluster](../../deployment/production-deployment/#instantiate-a-cluster).<br/><br/>Default: Null. |