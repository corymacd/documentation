...
### Instantiate Redpanda

There are two ways to instantiate Redpanda: explicit initialization (recommended) or implicit initialization (default). 

#### Explicit initialization (recommended)

The explicit initialization process enables easier deployment with simplified configuration of node IDs and seed servers. 

Redpanda automatically generates node IDs, so you don't need to include node IDs in config files or worry about policies on `node_id` re-use. Redpanda initially starts up without an ID, and the node gets an ID only after it joins the cluster. 

Additionally, all nodes can have identical `seed_servers` in their config file. The minimum number of seed servers to form a cluster is three, but you can adjust this to increase the robustness of consensus and minimize any chance that new clusters get spuriously formed after nodes are lost or restarted without any data. 

It's important to have one of more seed servers in each fault domain (such as rack, cloud AZ). A higher number provides a stronger guarantee that clusters don’t fracture unintentionally.

#### Implicit initialization (default)

Implicit initialization is supported for backward compatibility. This process is more complex. It requires that each node have a unique `node_id` set in its config file, and exactly one node must have `seed_servers` empty in its config file. 

You first configure and start the root node. The root node starts as a standalone node, and every other node joins it, forming a cluster along the way. You must be careful not to add new nodes with the same `node_id` as decommissioned nodes. For the root node, choose 0 as its ID. The `--self` option tells the node which interface address to bind to. Usually you want that to be its private IP.
