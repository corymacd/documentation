...
With `minimum_initial_cluster_size=3`, clusters start with three configurable nodes, instantiating a controller Raft group with the seed servers list. The cluster grows as nodes start up. 
...
- It's important to have one of more seed servers in each fault domain (such as rack or cloud AZ). A higher number provides a stronger guarantee that clusters don’t fracture unintentionally.
:::

#### Set cluster size to one

With `minimum_initial_cluster_size=1`, the process is more complex. This requires that each node has a unique `node_id` set in its configuration file, and exactly one node has an empty `seed_servers` set in its configuration file. A single (root) node instantiates the cluster, and other nodes join it.  
