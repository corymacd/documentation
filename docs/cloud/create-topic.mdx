...
5. (Optional) Fill in the following fields to overwrite the default settings. Defaults are populated from information stored in the broker.
    ...
    | **Cleanup Policy** | The policy that determines how old log segments are cleaned up. The choices are **delete** and **compact**. The default is **delete**. |
...
7. Click **Create**.
8. When the success message appears, click **Close**.
