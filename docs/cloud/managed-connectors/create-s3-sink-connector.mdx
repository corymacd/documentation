...
To create an S3 sink connector, first choose the connector type, then configure connector properties, then review and apply the connector configuration. To test the connection, produce a message to the topic and verify that the S3 bucket contains the message.
...
1. In **Connector name**, type a name for your connector.
2. In **Key converter class**, select a converter. Choices include:
    * org.apache.kafka.connect.storage.StringConverter
    * org.apache.kafka.connect.json.JsonConverter
    * org.apache.kafka.connect.converters.ByteArrayConverter
3. In **Topics**, type the name of the topic whose records will be exported to the S3 bucket. If more than one topic will be exported, provide a comma-separated list; for example, `topic1, topic2, topic3`.
...
5. Click **AWS** and fill in the following information (available from the AWS management console):
    ...
    * aws.s3.bucket.name
    * aws.s3.endpoint (you can leave this field empty)
    * aws.s3.region
    * aws.s3.part.size.bytes (you can use the default value, which is 5242880)
6. Click **File** and fill in the following information:
    * file.name.prefix
    * file.name.template (you can leave this field empty)
...
7. Click **Format** and fill in the following fields:
    * format.output.type (choices include `csv`, `json`, and `jsonl`)
...
4. Click the refresh symbol at the top of the page to display the newly created connector.

### Produce a message to a topic

After the connector is created, test the connection by writing to one of your topics and checking the contents of the S3 bucket in the AWS management console.

1. From the left menu, select **Topics**.
...
4. Select the **Key** tab and type a key on the first line. For example, type `key-1`.
5. Select the **Value** tab and type a value on the first line. For example, type `[{"name":"user-1"}]`.
6. Click **Publish**.
    A success message appears.
7. Click **Close** to close the message.
8. From the left menu, select **Kafka Connect**.
9. Under **Cluster**, click `connectors`.
    The **connectors** summary page opens, and the state for **Connector** shows `Running`.
