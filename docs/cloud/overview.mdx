...
* Dedicated Cloud
    You host your data in Redpanda’s Virtual Private Cloud (VPC), and Redpanda handles the provisioning, operations, and maintenance for you. Redpanda also performs automatic backups to S3 (for AWS) or GCS (for GCP).
...
Both methods offer the following options:
...
* Managed connectors (link to doc here)
...
There is no outbound connection from your VPC to the Redpanda VPC in the control or data path. The following architecture diagram illustrates the communication between the control plane and the data plane when you use a VPC in AWS:
